...
**Backend URL:** `http://172.16.200.136:4000`
...
```javascript
const API_BASE_URL = "http://172.16.200.136:4000";
```

**Important:**
- Use `172.16.200.136:4000` when testing on **mobile devices**
- Use `localhost:4000` when testing on **same computer** (web browser)
...
**Example:**
```javascript
const response = await fetch('http://172.16.200.136:4000/health');
...
**Example:**
```javascript
const response = await fetch('http://172.16.200.136:4000/api/reports');
const data = await response.json();
...
```javascript
const caseId = 'WP-001';
const response = await fetch(`http://172.16.200.136:4000/api/reports/${caseId}`);
...
const fileUrl = `http://172.16.200.136:4000/uploads/document-1730000000000-123456789.pdf`;
...
const socket = io('http://172.16.200.136:4000', {
